...
const authenticateToken = async (req, res, next) => {
  try {
    ...
    if (!tokenHeader) response.throwNewError(401, 'Oops! You are not authorized to access this resource 1')

    const tokenBearer = tokenHeader.split(' ')[0]
    const token = tokenHeader.split(' ')[1]

    if (tokenBearer !== 'Bearer') response.throwNewError(401, 'Oops! You are not authorized to access this resource 2')
    if (!token) response.throwNewError(401, 'Oops! You are not authorized to access this resource 3')
